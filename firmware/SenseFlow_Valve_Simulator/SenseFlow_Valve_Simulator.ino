...
void checkConfig() {
  ...
  if (Firebase.RTDB.getInt(&fbdo, (basePath + "minPercent").c_str())) {
    uint8_t v = (uint8_t)fbdo.intData();
  ...
  if (Firebase.RTDB.getBool(&fbdo, (basePath + "analyticsOn").c_str())) {
    bool v = fbdo.boolData();
    if (v != analyticsOn) { analyticsOn = v; Serial.printf("[CONFIG] analyticsOn=%s\n", analyticsOn ? "ON" : "OFF"); }
  }
}
    ...
    if (upper.startsWith("WIFI ")) {
      ...
      switch (cmd) {
        ...
        case 'S': case 's':
          ...
          Serial.println("------------------------\n");
          break;
        default: Serial.println("O=open C=close S=status WIFI ADMIN RESET_WIFI"); break;
      }
    }
  }
}
