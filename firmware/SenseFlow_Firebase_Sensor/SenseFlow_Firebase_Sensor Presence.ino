...
// Sensor logic: 0 = active HIGH (water=HIGH), 1 = active LOW (water=LOW/GND)
...
void initDipSensors() {
  for (int i = 0; i < SENSOR_COUNT; i++) {
      ...
      pinMode(DIP_PINS[i], INPUT_PULLUP);   // Pulled HIGH, sensor pulls to GND when water detected
    #else
      pinMode(DIP_PINS[i], INPUT_PULLDOWN);  // Pulled LOW, sensor pulls HIGH when water detected
    #endif
  }
}
...
uint8_t readDipRaw() {
  ...
  for (int i = 0; i < SENSOR_COUNT; i++) {
    #if SENSOR_ACTIVE_LOW
      if (digitalRead(DIP_PINS[i]) == LOW) {   // Active LOW — GND = water present
        bits |= (1 << i);
      }
    #else
