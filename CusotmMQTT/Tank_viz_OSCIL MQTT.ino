...
#define FIRMWARE_VERSION  "17.0.0"
#define FIRMWARE_CODE     "SF-OSC-MQTT-2026-01"
#define AP_PASSWORD       "mvstech9867"

// ── Local MQTT (Phase 1: publish only) ────────────
// ESP32 auto-discovers the gateway by UDP broadcast (no IP config needed).
// Works in parallel with Firebase — when internet is down, local dashboard
...
#define MQTT_DISCOVERY_INTERVAL_MS 60000   // re-broadcast every 60s if not found
#define MQTT_PUBLISH_INTERVAL_MS   2000    // also push every 2s on top of change-driven

// MQTT credentials — username = deviceCode, password = first 16 hex chars of
// SHA256(deviceCode + MQTT_SECRET). Pi-side script knows MQTT_SECRET and
// auto-registers any device that hits it via UDP discovery, so no manual
// password entry per device. If a device is replaced, the new deviceCode
// gets a new password automatically.
//
// IMPORTANT: keep MQTT_SECRET private — anyone with it can forge as any device
// on your LAN. Rotate by reflashing all devices + updating Pi script.
...
bool apAlwaysOn = true;            // keep AP up even after STA connects (default ON)
int   usFailCount = 0;
...
void loadOrCreateDeviceCode() {
  ...
  apAlwaysOn = prefs.getBool("apOn", true);
...
void processDipSensors() {
  ...
  // Rate-limit reads to once every DIP_READ_INTERVAL_MS — gives common pin
  // ~99.9% LOW duty cycle, dramatically slowing electrolytic corrosion.
  if (millis() - lastDipRead < DIP_READ_INTERVAL_MS) return;
  lastDipRead = millis();

  uint8_t currentRaw = readDipRaw();

  // Sync-sampled read is already self-filtered (8/10 agreement). Commit immediately.
  if (currentRaw != sensorBits) {
    sensorBits = currentRaw;
  }
  {
...
/* Tank visualization */
.tank-wrap{display:flex;gap:20px;align-items:stretch}
/* Tapered tank silhouette (top is narrower, like a real water tank with a lid) */
.tank{position:relative;width:150px;height:260px;flex-shrink:0}
.tank-svg{width:100%;height:100%;display:block}
.tank-water{transition:height .6s ease-out}
.probe{position:absolute;left:-22px;width:142px;height:0;border-top:2px dashed #cbd5e1;display:flex;align-items:center}
.probe.on{border-top-color:#2563eb}
.probe-tab{position:absolute;left:-30px;background:#e5e7eb;color:#64748b;font-size:10px;font-weight:700;padding:2px 6px;border-radius:4px;transform:translateY(-50%);white-space:nowrap}
.probe.on .probe-tab{background:#2563eb;color:#fff}
.tank-info{flex:1;display:flex;flex-direction:column;justify-content:center;gap:8px}
.big-pct{font-size:48px;font-weight:800;color:#2563eb;line-height:1}
.litres{font-size:18px;color:#475569;font-weight:600}
.tank-sub{font-size:12px;color:#64748b}
.err-banner{background:#faf5ff;border:1px solid #d8b4fe;color:#7e22ce;padding:8px 12px;border-radius:8px;font-size:12px;font-weight:600;margin-top:8px}
</style>
</head><body>
)rawliteral";
  ...
  #if !USE_ULTRASONIC
  {
    ...
    if (tankCapacityLitres > 0) {
      ...
      html += "<div class='litres' id='litres'>" + String(litres) + " / " + String(tankCapacityLitres) + " L</div>";
    } else {
      html += "<div class='litres' id='litres' style='display:none'></div>";
    }
    html += "<div class='tank-sub'>" + String(SENSOR_COUNT) + "-probe DIP &bull; bits " + String(sensorBits, BIN) + "</div>";
    if (sensorError) {
  ...
  html += "<div class='card'>";
  html += "<h2>Actions</h2>";
  html += "<div style='display:flex;align-items:center;gap:8px;padding:8px 0;font-size:13px'>";
  html += "<label style='display:flex;align-items:center;gap:6px;cursor:pointer;flex:1'>";
  html += "<input type='checkbox' id='apModeChk' " + String(apAlwaysOn ? "checked" : "") + " onchange=\"location.href='/setapmode?on='+(this.checked?'1':'0')\">";
  html += "<span>Keep AP mode active even after WiFi connects</span>";
  html += "</label></div>";
  html += "<p style='font-size:11px;color:#888;margin-bottom:8px'>When ON, you can always connect to this device's WiFi to view it locally.</p>";
  ...
  html += "function fmtL(p,c){return c>0?Math.round(p*c/100)+' / '+c+' L':''}";
...
void mqttBroadcastDiscovery() {
  ...
  bcast[3] = 255;   // assume /24 — sufficient for most home/society LANs
...
// Derive MQTT password = first 16 hex chars of SHA256(deviceCode + MQTT_SECRET).
// Pi-side auto-register script computes the same and stores it in Mosquitto's
// password file when it sees this device on UDP discovery.
...
void mqttLoop() {
  ...
  // Stay connected, push periodically
...
void setup() {
  ...
  mvs.addEndpoint("/setapmode", []() {
    WebServer* srv = mvs.getServer();
    bool on = (srv->arg("on") == "1");
    apAlwaysOn = on;
    Preferences apPrefs;
    apPrefs.begin("senseflow", false);
    apPrefs.putBool("apOn", on);
    apPrefs.end();
    Serial.printf("AP always-on set to: %s\n", on ? "ON" : "OFF");
    // Apply immediately: if turning OFF and STA is connected, drop AP
    if (!on && WiFi.status() == WL_CONNECTED) {
      WiFi.softAPdisconnect(true);
      WiFi.mode(WIFI_STA);
      Serial.println("AP dropped (STA-only mode now)");
    } else if (on) {
      WiFi.mode(WIFI_AP_STA);
      WiFi.softAP(apName.c_str(), AP_PASSWORD);
  ...
  if (mvs.hasSavedWiFi()) {
    ...
    if (mvs.connectToSavedWiFi(30)) {
      ...
      if (!apAlwaysOn) {
        WiFi.softAPdisconnect(true);
        WiFi.mode(WIFI_STA);
        Serial.println("AP dropped (apAlwaysOn=false)");
      }
      initFirebase();
    } else {
